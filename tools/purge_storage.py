...
if os.getenv("DELETABLE_DATE") is not None:
    ...
    DELETABLE_DATE = datetime.now() - timedelta(days=180)
DISKS = ["d:/", "e:/", "f:/"]
...
for disk in DISKS:
    ...
    print(
        f"{disk} used: {used / total * 100:.2f}%, deletable size: {deletable_size / (2**30):.2f} GB ({deletable_size / total * 100:.2f}%)"
    )
...
if user_input.lower() in ["y", "yes"]:
    ...
    for disk in DISKS:
        ...
        # Clean empty folders
        for folder in Path(f"{disk}/4drec_data").glob("*"):
            if folder.is_dir():
                stat = folder.stat()
                if stat.st_ctime >= DELETABLE_DATE.timestamp():
                    continue
